...
export function ResumeDocument({ forcedLang }: { forcedLang?: Lang }) {
  const context = useLang();
  const lang = forcedLang ?? context.lang;
  const t = forcedLang ? translations[forcedLang] : context.t;
  const cv = t.cv;
  ...
  return (
    <>
      ...
      <div className="cv-toolbar no-print sticky top-0 z-50 flex items-center justify-between px-6 py-3 border-b"
        style={{ background: "#050d1a", borderColor: "rgba(59,130,246,0.15)" }}>
        <Link href="/" className="cv-back-link flex items-center gap-2 transition-colors text-sm">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center text-xs font-medium tracking-widest" aria-label={t.ui.language}>
            <Link href="/cv/en" aria-current={lang === "en" ? "page" : undefined}>EN</Link>
            <span className="mx-2 text-slate-700" aria-hidden="true">/</span>
            <Link href="/cv/es" aria-current={lang === "es" ? "page" : undefined}>ES</Link>
          </div>
          <button
      ...
      <div className="cv-root px-8 py-10 min-h-screen"
        ...
        <section className="mb-6">
          ...
          <p className="text-sm leading-relaxed" style={{ color: "#374151" }}>
            {cv.profileText}{" "}
            <strong style={{ color: "#111827" }}>{cv.profileTextBold}</strong>
          </p>
        </section>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-5">
            ...
            <section>
              ...
              <ul className="space-y-1">
                {certs.certs.map((c) => (
                  <li key={c.name} className="flex gap-2 text-xs" style={{ color: "#374151" }}>
                    <span style={{ color: "#2563eb" }}>✓</span> {c.name}
                  </li>
                ))}
              </ul>
            </section>
            ...
            <section>
              <h2 className="text-xs font-black tracking-[0.18em] uppercase mb-3 pb-1 border-b" style={{ color: "#2563eb", borderColor: "#e5e7eb" }}>
                {certs.languagesTitle}
              </h2>
              <div className="space-y-1 text-xs" style={{ color: "#374151" }}>
                {certs.languages.map((l) => (
                  <p key={l.lang}><strong style={{ color: "#111827" }}>{l.lang}</strong> — {l.level}</p>
                ))}
              </div>
            </section>
          </div>
